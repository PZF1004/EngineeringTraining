...
public class AddStudentServlet3 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (isMultipart) { // 判断前台的form是否有 mutipart属性
				...
				boolean result = service.addStudent2(student);// service返回值传给servlet

				if (result) {
					out.println("文件增加成功");
					System.out.println("成功！");
					response.setHeader("refresh", "3;URL=QueryAllStudentByPageServlet");
				} else {
					out.println("文件增加失败");
